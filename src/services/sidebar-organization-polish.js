(function () {
  ...
  function squadsOpen() {
    return sessionStorage.getItem(SQUADS_OPEN_KEY) === '1';
  }

  function setSquadsOpen(value) {
    sessionStorage.setItem(SQUADS_OPEN_KEY, value ? '1' : '0');
  }
  ...
  function ensureStyle() {
    ...
    style.textContent = `
      ...
      .v4-kommo-fixed{border-top:1px solid rgba(255,255,255,.10);padding-top:10px;margin-top:auto}.v4-kommo-clients-panel,.v4-kommo-squads-panel{display:none;padding:7px 0 16px}.v4-kommo-clients-panel.open,.v4-kommo-squads-panel.open{display:block}.v4-kommo-client{min-height:58px;border-radius:15px;margin-bottom:7px;background:rgba(255,255,255,.045);border:1px solid rgba(255,255,255,.07)}.v4-kommo-client.active{background:linear-gradient(135deg,rgba(207,16,34,.34),rgba(255,255,255,.05));border-color:rgba(207,16,34,.40)}.v4-kommo-avatar{width:34px;height:34px;border-radius:12px;display:grid;place-items:center;background:var(--client-color,#cf1022);font-weight:1000;color:#fff;flex:0 0 auto}.v4-kommo-squad{min-height:54px;border-radius:15px;margin-bottom:7px;background:rgba(255,255,255,.045);border:1px solid rgba(255,255,255,.07)}.v4-kommo-squad.active{background:linear-gradient(135deg,rgba(207,16,34,.34),rgba(255,255,255,.05));border-color:rgba(207,16,34,.40)}.v4-kommo-squad-avatar{width:34px;height:34px;border-radius:12px;display:grid;place-items:center;background:linear-gradient(135deg,#cf1022,#570610);font-weight:1000;color:#fff;flex:0 0 auto}.v4-kommo-divider{height:1px;background:rgba(255,255,255,.10);margin:8px 0}.v4-kommo-all-clients,.v4-kommo-all-squads{background:rgba(255,255,255,.075);border:1px solid rgba(255,255,255,.09);margin-bottom:8px}
    `;
  }
  ...
  function renderSquadSelector(clients) {
    const squads = getSquads();
    if (!squads.length) return '';
    const open = squadsOpen();
    return '<div class="v4-kommo-section"><div class="v4-kommo-label">Equipes</div>' +
      '<button class="v4-kommo-item ' + (open ? 'active' : '') + '" type="button" data-v4-squad-toggle><span class="ico">▦</span><span><strong>Squads</strong><small>Selecionar equipe ou ver geral</small></span><span class="v4-kommo-counter">' + escapeHtml(squads.length) + '</span><span class="chev">' + (open ? '⌃' : '⌄') + '</span></button>' +
      '<div class="v4-kommo-squads-panel ' + (open ? 'open' : '') + '">' +
      '<button class="v4-kommo-item v4-kommo-all-squads" type="button" data-v4-open-organization><span class="ico">◎</span><span><strong>Geral / Todos os squads</strong><small>Gerenciar equipes e acessos</small></span></button>' +
      squads.slice(0, 12).map(function (squad) {
        ...
        return '<button class="v4-kommo-item v4-kommo-squad" type="button" ' + (firstClient ? 'data-client="' + escapeHtml(firstClient) + '"' : 'data-v4-open-organization') + '><span class="v4-kommo-squad-avatar">' + escapeHtml(initials(squad.name)) + '</span><span><strong>' + escapeHtml(squad.name) + '</strong><small>' + escapeHtml(squad.headEmail || 'Sem head definido') + '</small></span><span class="v4-kommo-counter">' + escapeHtml(count) + '</span></button>';
      }).join('') + '</div></div>';
  }
  ...
  function start() {
    ...
    log('sidebar_polish', 'Sidebar V4 com seletores recolhiveis de squads e clientes, sem ajuda.');
  }

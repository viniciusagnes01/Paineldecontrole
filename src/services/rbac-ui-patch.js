(function () {
  const TAB_LABELS = {
    ...
    ads: 'Mídia & Ads',
    crm: 'CRM & Funil',
    competitors: 'Concorrentes SEMrush',
    goals: 'Metas',
    status: 'Status do Projeto',
    'client-config': 'Config do Cliente',
  ...
  function orgPanelTemplate() {
    ...
    return `<section class="v4-org-panel" data-v4-org-panel><header><div><h2>Organização e acessos</h2><p>RBAC modular: cargo define acesso, acesso define painel, painel define ação permitida.</p></div><button class="ghost" data-v4-org-close>Fechar</button></header><div style="padding:18px"><p><strong>Usuário atual:</strong> ${escapeHtml(current?.name || '-')} • ${escapeHtml(roleLabel(current?.role))}</p><table><thead><tr><th>Nome</th><th>E-mail</th><th>Cargo</th><th>Clientes</th></tr></thead><tbody>${users.map((u) => `<tr><td>${escapeHtml(u.name || '-')}</td><td>${escapeHtml(u.email)}</td><td>${escapeHtml(roleLabel(u.role))}</td><td>${escapeHtml((u.clientIds || []).join(', ') || '-')}</td></tr>`).join('')}</tbody></table><h3>Adicionar/alterar acesso local</h3><form data-v4-rbac-form style="display:grid;grid-template-columns:1fr 1fr 1fr auto;gap:10px;align-items:end"><label>Nome<input name="name" placeholder="Nome" /></label><label>E-mail<input name="email" placeholder="email@dominio.com" required /></label><label>Cargo<select name="role">${roleOptions}</select></label><label>Clientes<input name="clientIds" placeholder="*, st1, prime" /></label><button type="submit">Salvar acesso</button></form><p style="font-size:12px;color:#aeb4c4">Primeira versão salva permissões locais do painel. Próximo ciclo: persistir no Supabase com roles, permissions e user_client_access.</p></div></section>`;
  }

  function mountOrgPanel() {
    if (document.querySelector('[data-v4-org-panel]')) return;
    const wrap = document.createElement('div');
    wrap.innerHTML = orgPanelTemplate();
    ...
    document.querySelector('[data-v4-rbac-form]')?.addEventListener('submit', function (event) {
      ...
      window.V4_RBAC.setUserRole(data.get('email'), data.get('role'), data.get('clientIds'), { name: data.get('name') });
      document.querySelector('[data-v4-org-panel]')?.remove();
      mountOrgPanel();
      document.querySelector('[data-v4-org-panel]')?.classList.add('open');
      toast('Acesso atualizado localmente.');
    });
  }
  ...
  function start() {
    ...
    log('rbac_ui', 'RBAC UI patch ativo.');
  }
